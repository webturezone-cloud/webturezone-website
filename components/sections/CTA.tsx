...
export function CTA({ heading, sectionHeadingSize, bodyTextSize }: CTAProps = {}) {
  const fallback = `${CTA_SECTION.headlineLine1} ${CTA_SECTION.headlineLine2}`;
  const headline = heading?.trim() || fallback;
  const headlineSize = sectionHeadingSize?.trim() || FALLBACK_HEADLINE;
  const bodySize = bodyTextSize?.trim() || 'text-sm sm:text-[0.9rem]';
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
          <div className="mx-auto w-full max-w-lg text-left lg:mx-0 lg:max-w-none">
            ...
            >
              <SplitText
                text={headline}
                className={`justify-center font-display uppercase leading-[1.05] tracking-tight text-white text-balance lg:justify-start ${headlineSize}`}
                from="bottom"
                splitBy="words"
                delay={0}
              />
            ...
            <p
              className={`mt-4 max-w-md text-left leading-relaxed text-slate-300 text-balance lg:mt-5 ${bodySize}`}
            >
              {CTA_SECTION.subtext}
            </p>
            <ul className="mt-6 flex max-w-md flex-col items-start gap-3 text-left sm:mt-7">
              {TRUST_POINTS.map((point) => (
                <li
                  key={point}
                  className={`flex items-start gap-2.5 leading-snug text-white/75 ${bodySize}`}
                >
